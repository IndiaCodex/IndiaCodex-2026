/**
 * In-memory fake implementations of every application port
 * (docs/architecture/cross-cutting-concerns.md#testing-strategy) — the
 * shared fakes every plugin, storage adapter, and interface's own test
 * suite is expected to build on instead of re-implementing its own.
 */
...
export class InMemorySnapshotRepository implements SnapshotRepositoryPort {
  ...
  /** Test-only convenience: how many snapshots have been saved so far. */
  public count(): number {
