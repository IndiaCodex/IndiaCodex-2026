...
exports.endStudySession = exports.startStudySession = exports.getDashboard = void 0;
...
const getDashboard = async (req, res) => {
    try {
        ...
        const studySessionsToday = await db_1.default.studySession.findMany({
            where: {
                studentId,
                startTime: { gte: today },
            },
        });
        const secondsStudiedToday = studySessionsToday.reduce((acc, session) => acc + (session.durationSeconds || 0), 0);
...
const startStudySession = async (req, res) => {
    try {
        ...
        const session = await db_1.default.studySession.create({
            data: {
                studentId,
...
const endStudySession = async (req, res) => {
    try {
        ...
        const session = await db_1.default.studySession.findUnique({
            where: { id: sessionId },
        });
        ...
        const durationSeconds = Math.max(Math.round((endTime.getTime() - session.startTime.getTime()) / 1000), 1);
        ...
        const updatedSession = await db_1.default.studySession.update({
            where: { id: sessionId },
            data: {
                endTime,
                durationSeconds,
