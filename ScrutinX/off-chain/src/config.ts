...
export const config = {
  ...
  // --- Congestion model ---
  ewmaAlpha: 0.3, // larger = reacts faster / noisier
  recentBlocks: 10, // window used to seed the EWMA on startup
  pollIntervalMs: 20_000, // ~Cardano block time
  ...
  // Max claims per settlement tx. MEASURED ON-CHAIN: spend runs once PER input × O(n²) each → ~O(n³).
  // 5 confirmed fine; 16 blew the mem budget (failed ~input 7) → real N_max ≈ 6–7. batchCap=6 is safe.
  // Dict-based O(1) lookups (→ ~O(n²) total) raise this materially. See onchain-spec.md §6.
  batchCap: 6,

  // Naive-side "fees saved" baseline: the REAL measured single-claim fee on Preprod (238,189 lovelace).
